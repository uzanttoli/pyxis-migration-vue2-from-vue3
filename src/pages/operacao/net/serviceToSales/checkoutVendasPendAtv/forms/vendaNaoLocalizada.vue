        ...
        <validation-observer v-slot="{ invalid, validate }" ref="observer">
          <form @submit.prevent="validate().then(submit)">
            ...
            <fieldset>
              ...
              <div style="display: flex; flex-wrap: wrap">
                <v-col cols="4" class="py-0">
                  <validation-provider name="venda localizada" rules="required" v-slot="{ errors }">
                    <v-select
                      outlined
                      placeholder="Venda localizada?"
                      label="Venda localizada?"
                      :error-messages="errors"
                      dense
                      :items="['SIM', 'NAO']"
                      v-model="formCheck.vendaLocalizada"
                    ></v-select>
                  </validation-provider>
                </v-col>
                <v-col cols="4" class="py-0" v-if="formCheck.vendaLocalizada == 'SIM'">
                  <validation-provider
  ...
  export default {
    ...
    methods: {
      ...
      salvarDados() {
        ...
        this.formCheck.statusCheck = 3 //default
        this.formCheck.tabulador = 2
      ...
      async submit() {
        try {
          if (this.dadosNovoCaso?.NUM_CASO) {
            if (this.formCheck.vendaLocalizada == 'SIM') {
              if (
                this.formCheck.img1 ||
                this.formCheck.img2 ||
                this.formCheck.img3 ||
