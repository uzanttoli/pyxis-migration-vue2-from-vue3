...
<script>
  ...
  export default {
    ...
    mounted() {
      this.gerarGraficoMinhaEvolucao()
    ...
    computed: {
      minhaEvolucao() {
        let minhaEvolucao = this.$store.getters.minhaEvolucao
        minhaEvolucao.map(item => {
    ...
    methods: {
      loadMinhaEvolucao() {
        this.$store.dispatch('loadMinhaEvolucao', this.usuarioAlmope)
      },
      gerarGraficoMinhaEvolucao() {
        let chart = am4core.create(this.$refs.chartdiv, am4charts.XYChart)
        let data = []

        for (let i = 0; i < this.minhaEvolucao.length; i++) {
          switch (this.indicadorSelecionado) {
            case 'ATENDIDAS':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].ATENDIDAS
              })
              break
            case 'TMT':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].TMT
              })
              break
            case 'RECHAMADAS_P':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].RECHAMADAS_P
              })
              break
            case 'LOGADO_CTI':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].LOGADO_CTI
              })
              break
            case 'JACKIN_CHE_CTI':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].JACKIN_CHE_CTI
              })
              break
            case 'RECHAMADA_72H':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].RECHAMADA_72H.toFixed(2).concat(this.porcentage)
              })
              break
            case 'DESCONEXAO':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].DESCONEXAO.toFixed(2).concat(this.porcentage)
              })
              break
            case 'SHORTCALL':
              data.push({
                dataRef: this.minhaEvolucao[i].DATA.substr(0, 10).split('-').reverse().join('/'),
                resultadoRef: this.minhaEvolucao[i].SHORTCALL.toFixed(2).concat(this.porcentage)
              })
              break
            default:
              break
