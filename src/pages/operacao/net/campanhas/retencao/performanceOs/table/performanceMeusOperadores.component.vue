<template>
  <!-- MEUS OPERADORES -> COORDENADOR E SUPERVISOR -->
  <div v-if="performanceOsMeusOperadores != ''">
    <v-card>
      ...
      <v-data-table
        ...
        :loading="loading"
        :items="performanceOsMeusOperadores"
        dense
        class="text-no-wrap"
        :headers="getHeaders()"
      >
...
<script>
  export default {
    name: 'TablePerformanceOsMeusOperadores',
    props: {
      actionUrlMeusOperadores: {
        type: String,
        require: true
      },
      sizeProgress: {
    ...
    computed: {
      performanceOsMeusOperadores() {
        ...
        let percent = '%'
        performanceOsMeusOperadores.map(item => {
          if (item['1']) {
        ...
        return performanceOsMeusOperadores
      }
    },
    methods: {
      loadPerformanceOsMeusOperadores() {
        this.$store.dispatch('loadPerformanceOsMeusOperadores', this.actionUrlMeusOperadores)
      },
      getHeaders() {
        let arrHeader = [
          {
    ...
    created() {
      this.loadPerformanceOsMeusOperadores()
    }
  }
</script>
