<template>
  <!-- MINHA OS [COORDENADOR, SUPERVISOR, OPERADOR] -->
  <v-card v-if="minhaPerformanceOs != ''">
...
<script>
  ...
  export default {
    ...
    computed: {
      minhaPerformanceOs() {
        let minhaPerformanceOs = this.$store.getters.minhaPerformanceOs
        let percent = '%'

        minhaPerformanceOs.map(item => {
          if (item['1']) {
    ...
    methods: {
      loadMinhaPerformanceOs() {
        this.$store.dispatch('loadMinhaPerformanceOs', this.performanceUrl)
      },
      // loadBase() {
      //   let urlData = `${config.baseUrl}${this.performanceUrl}`;
      ...
      getHeaders(cargoColaborador = this.cargoColaboradorLogado) {
        ...
        if (cargoColaborador == 'GERENTE') {
          ...
          arrHeader.unshift(headerOperador)
        } else {
          this.colunaChave = 'Nome'
          arrHeader.unshift({
            text: 'Nome',
