  ...
  <v-card elevation="1">
    <v-col>
      ...
      >
        ...
        <template v-slot:item.SHORTCALL_MM2="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM2, item.CHAMADAS_MM2) }}
        </template>
        <template v-slot:item.SHORTCALL_MM1="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM1, item.CHAMADAS_MM1) }}
        </template>
        <template v-slot:item.SHORTCALL_MM0="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM0, item.CHAMADAS_MM0) }}
        </template>
        <template v-slot:item.SHORTCALL_MM0_DM3="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM0_DM3, item.CHAMADAS_MM0_DM3) }}
        </template>
        <template v-slot:item.SHORTCALL_MM0_DM2="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM0_DM2, item.CHAMADAS_MM0_DM2) }}
        </template>
        <template v-slot:item.SHORTCALL_MM0_DM1="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM0_DM1, item.CHAMADAS_MM0_DM1) }}
        </template>
        <template v-slot:item.SHORTCALL_MM0_DM0="{ item }">
          {{ calcularPercentual(item.SHORTCALL_MM0_DM0, item.CHAMADAS_MM0_DM0) }}
        </template>
        <!-- rodape -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      loadTotalBase() {
        let urlData = `${config.baseUrl}api/shared/workflow/total_shortcall60_coordenador/${this.produtoAtual}`
        this.$api.get(urlData).then(res => {
