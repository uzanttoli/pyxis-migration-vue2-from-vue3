<template>
  <div class="card">
    ...
    <div class="content">
      <span class="title">{{ arquiveTitle }}</span>
      <div class="desc">
        <p>{{ nameArquive }}</p>
      </div>
      <div :class="isDelete ? 'actions' : 'actions-center'">
        <a :href="arquivoWs(attachment)" class="download">Baixar arquivo</a>
        <!-- <a href="#" class="notnow" v-if="isDelete">Cancelar envio</a> -->
        <button class="notnow" v-if="isDelete" @click="$emit('click-cancel-arquive')">
          Excluir
        </button>
      </div>
    </div>
