  ...
  <v-sheet>
    <v-col :cols="agrupamentosProduto.length + 1" class="py-2">
      <v-toolbar class="elevation-0" dense>
        <v-btn-toggle
          v-model="agrupamento"
          @change="loadConfigControleDeslogue()"
          shaped
          mandatory
    ...
    <v-row>
      <v-col cols="12" md="12" v-for="(item, i) in controleDeslogueConfig" :key="i">
        <component
          :is="item.COMPONENT"
          :key="i"
          :title="item.TITULO"
...
<script>
  ...
  import tabelaControleDeslogue from '../controleDeslogue/table/controleDeslogue'

  export default {
    name: 'controle-deslogue-component',
    mixins: [strGetParamMixin],
    components: {
      tabelaControleDeslogue
    },
    data: function () {
      return {
        loading: false,
    ...
    computed: {
      ...
      agrupamento: {
        get: function () {
          return this.$store.getters.controleDeslogueAgrupamento
        },
        set: function (newValue) {
          this.$store.dispatch('setAgrupamentoControleDeslogue', newValue)
        }
      },
      produtoAtual: {
        get: function () {
      ...
      agrupamentosProduto() {
        return this.$store.getters.controleDeslogueAgrupamentos.map(element => element.AGRUPAMENTO)
      }
    },
    methods: {
      loadConfigControleDeslogue() {
        this.$store.dispatch('loadConfigControleDeslogue', {
          produto: this.produtoAtual,
          agrupamento: this.agrupamento
        })
      },
      loadProdutos() {
        this.$store.dispatch('loadProdutosControleDeslogue')
      },
      ...
      async loadAgrupamentosPorProdutos() {
        this.loading = true
        await this.$store.dispatch('loadAgrupamentosControleDeslogue', this.produtoAtual)
      },

      async reloadData() {
        await this.loadAgrupamentosPorProdutos()
        await this.loadConfigControleDeslogue()
      }
    },
    created() {
      this.loadProdutos()
