<template>
  <!-- v-model="abrirModalFeedback" -->
  <v-dialog
    :model-value="abrirModalFeedback"
    @update:modelValue="val => $emit('update:abrirModalFeedback', val)"
    width="50%"
    persistent
  >
    <v-card>
      <v-card-title>
        <div v-if="dados.PERMITIR_LIBERACAO == 'REPROVADO'">Nova validação</div>
        <div v-else>Solicitar Benção</div>
        <v-spacer></v-spacer>
      ...
      <v-col v-show="dados.PERMITIR_LIBERACAO == 'REPROVADO'">
        <div class="dados-validacao">
          <div class="text-center">
            Tratado por:
            <v-chip class="ma-2" color="indigo" text-color="white" small>
              {{ dados.NOME_VALIDACAO }}
            </v-chip>
          </div>
          <div class="text-center">
            Status Benção:
            <v-chip class="ma-2" color="red" text-color="white" small>
              {{ dados.PERMITIR_LIBERACAO }}
            </v-chip>
          </div>
        </div>
        <v-textarea
    ...
    methods: {
      ...
      async submitFiles() {
        try {
          ...
          this.$emit('update:solicitacao-enviada', true)
      ...
      async solicitarBencao() {
        try {
          let data = {
            ...
            // evidenciaBencao2: this.$refs?.files?.files[1]?.name,
