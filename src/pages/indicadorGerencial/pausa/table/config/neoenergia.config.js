function GetNeoEnergiaConfigTable(pNumSuperior, coordFilter, supFilter) {
  ...
  const headersBase = [
    ...
    createHeader('Alimentação', 'ALIMENTACAO'),
    createHeader('Particular', 'PARTICULAR'),
    createHeader('Descanso', 'DESCANSO'),
    createHeader('Sistema', 'SISTEMA'),
    createHeader('Treinamento', 'TREINAMENTO'),
    createHeader('Feedback', 'FEEDBACK'),
    createHeader('Saúde', 'SAUDE'),
  ...
  const headersSupervisao = {
    COORDENADOR: createHeader('Coordenador', 'COORDENADOR', value => {
  ...
  switch (pNumSuperior) {
    case 1:
      finalHeaders.unshift(headersSupervisao.COORDENADOR)
      break
    case 2:
...
export default GetNeoEnergiaConfigTable
